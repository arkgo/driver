...
type (
	...
	defaultHttpConnect struct {
		...
		config ark.HttpConfig

		server  *http.Server
		router  *mux.Router
		handler ark.HttpHandler

		routes    map[string]*mux.Route
		registers map[string]ark.HttpRegister
	}
...
//连接
func (driver *defaultHttpDriver) Connect(config ark.HttpConfig) (ark.HttpConnect, error) {
...
func (connect *defaultHttpConnect) Health() (ark.HttpHealth, error) {
	//connect.mutex.RLock()
	//defer connect.mutex.RUnlock()
	return ark.HttpHealth{Workload: connect.actives}, nil
}
...
//注册回调
func (connect *defaultHttpConnect) Accept(handler ark.HttpHandler) error {
	connect.mutex.Lock()
...
//订阅者，注册事件
func (connect *defaultHttpConnect) Register(name string, config ark.HttpRegister) error {
	connect.mutex.Lock()
	defer connect.mutex.Unlock()
	...
	// 			if ark.Mode == Developing && connect.config.Port != 80 {
	// 				host = fmt.Sprintf("%v:%v", host, connect.config.Port)
	// 			}

	// 			//ark.Debug("http.reg", routeName, host)
...
func (connect *defaultHttpConnect) ServeHTTP(res http.ResponseWriter, req *http.Request) {
	...
	//ark.Debug("serve", name, site, params)

...
func (table *PostgresTable) Create(data Map) Map {
	...
	//按字段生成值
	value := Map{}
	errm := ark.Mapping(table.fields, data, value, false, false)
	if errm != nil {
	...
	//触发器
	table.base.trigger(ark.DataCreateTrigger, Map{"base": table.base.name, "table": table.name, "entity": value})
...
func (table *PostgresTable) Change(item Map, data Map) Map {
	...
	//按字段生成值
	value := Map{}
	errm := ark.Mapping(table.fields, data, value, true, false)
	if errm != nil {
	...
	table.base.trigger(ark.DataChangeTrigger, Map{"base": table.base.name, "table": table.name, "entity": newItem, "before": item, "after": newItem})
...
//		table.base.trigger(ark.DataRemoveTrigger, Map{"base": table.base.name, "table": table.name, "entity": items[0], "entities": items})
//	} else {
//		ark.Trigger(ark.DataRemoveTrigger, Map{"base": table.base.name, "table": table.name, "entity": items[0], "entities": items})
//	}
//
//	return affected
//
...
//	//注意这里，如果手动提交事务， 那这里直接返回，是不需要提交的
//	if table.base.manual {
//		table.base.trigger(ark.DataRecoverTrigger, Map{"base": table.base.name, "table": table.name, "entity": items[0], "entities": items})
//	} else {
//		ark.Trigger(ark.DataRecoverTrigger, Map{"base": table.base.name, "table": table.name, "entity": items[0], "entities": items})
//	}
//
//	return affected
//}
...
func (table *PostgresTable) Remove(args ...Any) Map {
	...
	//注意这里，如果手动提交事务， 那这里直接返回，是不需要提交的
	table.base.trigger(ark.DataRemoveTrigger, Map{"base": table.base.name, "table": table.name, "entity": item})
...
func (table *PostgresTable) Update(update Map, args ...Any) int64 {
	...
	//按字段生成值
	value := Map{}
	errm := ark.Mapping(table.fields, update, value, true, false)
	if errm != nil {
